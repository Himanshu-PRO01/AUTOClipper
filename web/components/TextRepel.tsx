...
function RepelLetter({
    ...
    if (letter === " ") {
        return <span className="text-repel-letter"> </span>;
    }

    return (
        <motion.span
            ref={ref}
            className={`text-repel-letter ${className || ""}`.trim()}
...
export function TextRepel({
    ...
    return (
        <div
            ref={containerRef}
            data-text-repel
            className={`text-repel-container ${className}`.trim()}
            onMouseMove={(e) => {
